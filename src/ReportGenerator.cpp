...
ReportGenerator::ReportGenerator() : operatorName("Operator") {
    updateTimestamp();
}
...
void ReportGenerator::updateTimestamp() {
    time_t now = time(0);
    struct tm timeInfo;
    localtime_s(&timeInfo, &now);  // Это работает в Visual Studio
    std::stringstream ss;
...
std::string ReportGenerator::generateReport(const ClayCalculator& calculator, const std::string& complianceStatus) {
    ...
    for (size_t i = 0; i < determinations.size(); ++i) {
        ss << std::setw(3) << (i + 1) << " | ";
        ss << std::setw(15) << std::fixed << std::setprecision(2) << determinations[i].sampleMass << " | ";
        ss << std::setw(13) << std::fixed << std::setprecision(2) << determinations[i].clayMass << " | ";
        ss << std::setw(17) << std::fixed << std::setprecision(2) << determinations[i].clayContent << "\n";
    }

    ss << "--------------------------------------------------------------------------------\n\n";
    ss << "STATISTICS:\n";
    ss << "Average clay content: " << std::fixed << std::setprecision(2) << calculator.getAverageContent() << "%\n";
    ss << "Minimum value: " << std::fixed << std::setprecision(2) << calculator.getMinContent() << "%\n";
    ss << "Maximum value: " << std::fixed << std::setprecision(2) << calculator.getMaxContent() << "%\n";
